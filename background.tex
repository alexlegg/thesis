...
Determining the existence of a winning controller strategy is a hard problem. In the general case, synthesis of an LTL formula, the complexity is double exponential. Synthesis suffers from a \emph{state explosion} problem. In the decisively titled \emph{Symbolic model checking: $10^{20}$ states and beyond}~\cite{Burch90} the authors claim an increase in the size of systems that can be checked from $10^3$--$10^6$ to $10^{20}$. This breakthrough result was achieved via symbolic, as opposed to explicit, representation of the states in the game structure.
...
\subsection{Solving Symbolic Games}
