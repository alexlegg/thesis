...
\begin{figure}
    \begin{tikzpicture}
        \node [thick,circle,draw] (s0){$x_0$};
        \node [thick,circle,draw,above right=0.6cm and 1cm of s0] (s1){$x_1$};
        \node [thick,circle,draw,below right=0.6cm and 1cm of s0] (s2){$x_2$};
        \node [thick,circle,draw,right=2cm of s2] (s3){$x_3$};
        \node [thick,circle,draw,right=2cm of s1] (s4){$x_4$};

        \draw [pil] (s0) edge node [above left] {$(i, 0)$} (s1);
        \draw [pil] (s0) edge node [below left] {$(i, 1)$} (s2);
        \draw [pil] (s1) edge node [above] {$(a, 0)$} (s4);
        \draw [pil] (s2) edge node [below] {$(b, 0)$} (s3);
        \draw [pil] (s3) edge node [right] {$(c, 0)$} (s4);
        \draw [pil] (s4) edge [loop above] node [above] {$(i, 0)$} (s4);
    \end{tikzpicture}
        ...
        \begin{tikzpicture}[dash pattern = on 2pt off 2pt, level distance = 10mm,baseline]
            \node [circle,draw] (root){}
                child {node [circle,draw] (left0){}
                    ...
                    edge from parent [fixed] node [left] {$a_0$}
                }
                child {node [circle] {}
                ...
                child {node [circle,draw] (right0){}
                    ...
                    edge from parent [fixed] node [right] {$a_n$}
                }
                node [left=4pt] {}
                node [above=4pt] {$\langle s, \kappa \rangle$};

            \draw [fixed] (left1.center) -- (left2.center);
            \draw [fixed] (left0) -- (left1.center);
            \draw [fixed] (left0) -- (left2.center);
            \draw [fixed] (right1.center) -- (right2.center);
            \draw [fixed] (right0) -- (right1.center);
            \draw [fixed] (right0) -- (right2.center);
        \end{tikzpicture}
        \end{minipage}
        \caption{Before}
    \end{subfigure}\hfill%
