...
%%%$\{ s' \mid s' \in 2^{\cS'} \land \delta(2^\cS, 2^\cU, 2^\cC, 2^{\cS'}) \land \sigma \land (\nex = 0) \}$, which evaluates to $s' = (\clk = 1 \land \err = 0)$.
...
\begin{algorithm}
   ...
   \begin{algorithmic}[1]
        ...
        \Function{GenLocalStrats}{$T$, $k$, $s$}
            ...
            \State $[(T_1,\sigma_1),\ldots,(T_j, \sigma_j)] \gets \Call{partition}{T, k, s \land \neg E(2^{\mathcal{S}_T})}$
...
The \textsc{partition} function (Algorithm~\ref{alg:strat:partition}) computes a local strategy in the root of an abstract game tree.  It takes a tuple $(T, k, s)$, such that $T$ is a certificate tree of height $k$ for states $s$ and partitions $\sigma = s(\cSU)$ into subsets $\sigma_i$ such that the controller can win a game of $k$ rounds in the states and against the environment actions contained in $\sigma_i$ by choosing action $c_i$.
...
\begin{proof}
    \begin{align*}
        ...
        A \land B ={}& \hat{\sigma} \land \bigg( E(2^{\mathcal{S}_{\tilde{T}})} \lor \\
                     & \pushright{ \bigvee_{\langle e, n \rangle \in \textsc{succ}(\tilde{T})} \delta(2^{\mathcal{S}_{\tilde{T}}}, 2^{\mathcal{U}_{\tilde{T}}}, 2^{\mathcal{C}_{\tilde{T}}}, 2^{\mathcal{S}_n}) \land \textsc{action}(e) \land \opptf(k, n) \bigg)} \\
                     & \pushright{\land \bigg( E(2^{\mathcal{S}_{T_i}}) \lor (\delta(2^{\mathcal{S}_{T_i}}, 2^{\mathcal{U}_{T_i}}, 2^{\mathcal{C}_{T_i}}, 2^{\mathcal{S}_{n_i}}) \land \textsc{action}(e_i) \land \opptf(k, n_i)) \bigg)} \\
        \implies{}& \hat{\sigma} \land \bigg( E(2^{\mathcal{S}_{\hat{T}}}) \lor \\
                  & \pushright{  \bigvee_{\langle e, n \rangle \in \textsc{succ}(\hat{T})} \delta(2^{\mathcal{S}_{\hat{T}}}, 2^{\mathcal{U}_{\hat{T}}}, 2^{\mathcal{C}_{\hat{T}}}, 2^{\mathcal{S}_n}) \land \textsc{action}(e) \land \opptf(k, n) \bigg)} \\
...
\end{proposition}
\begin{proof}
    We prove by induction. It is a precondition of the function that $T$ is a certificate tree for $s$, thus the invariant holds for the initial values $\hat{T} = T$ and $\hat{\sigma} = s(\cSU)$.  By the induction hypothesis $(\hat{\sigma} \land \opptf(k, \hat{T}))$ is unsatisfiable, so by Proposition~\ref{prop:aandb} $(A \land B)$ must also be unsatisfiable.  Hence the interpolation operation in line~\ref{alg:partition:I} is well defined.  By the properties of interpolants, $(A \implies \mathcal{I})$, hence $(\neg \mathcal{I} \implies \neg A)$ or equivalently $(\neg\mathcal{I} \implies \neg (\hat{\sigma} \land \opptf(k, \tilde{T}))$.
...
\begin{proof}
    ...
    Thus, 
    \begin{align*}
        \sigma_i \land \opptf(k, T_i) ={}& \mathcal{I}(2^{\cS_T} \times 2^{\cU_T}) \land \hat{\sigma} \land \opptf(k, T_i) \\
        ={}& \mathcal{I} \land \hat{\sigma} \land \opptf(k, T_i)
    \end{align*}
    By the properties of interpolants, $\mathcal{I} \land B = \mathcal{I} \land \opptf(k, T_i) = \bot$.
    Hence $\sigma_i \land \opptf(k, T_i) = \bot$, i.e., $T_i$ is a certificate tree for $\sigma_i$.
\end{proof}
...
\begin{proposition}\label{prop:next}
    ...
    Then:
    \begin{enumerate}
        \item $\mathcal{I}$ is an overapproximation of the $c$-successors of $\sigma$, i.e., $\mathcal{I} \supseteq s'$ where $$s' = \exists 2^\cS \exists 2^\cU \exists 2^\cC (\delta(2^\cS, 2^\cU, 2^\cC, 2^{\cS'}) \land \sigma \land c)$$
